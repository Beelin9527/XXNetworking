...
+ (XXCacheManager *)shareManager {
    static XXCacheManager *_XXCacheManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _XXCacheManager = [[XXCacheManager alloc] init];
    });
    return _XXCacheManager;
}
...
- (void)cacheResponseObject:(id)responseObject
    ...
    if (error == nil) {
        //缓存到内存中
        [XXMemoryCache writeData:responseObject forKey:hash];
        ...
        if (!directoryPath) {
            directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"XXNetworking"] stringByAppendingPathComponent:@"networkCache"];
            [[NSUserDefaults standardUserDefaults] setObject:directoryPath forKey:cacheDirKey];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
        [XXDiskCache writeData:data toDir:directoryPath filename:hash];
        [[XXLRUManager shareManager] addFileNode:hash];
    }
...
- (id)getCacheResponseObjectWithRequestUrl:(NSString *)requestUrl
    ...
    //先从内存中查找
    cacheData = [XXMemoryCache readDataWithKey:hash];
    
    if (!cacheData) {
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        
        if (directoryPath) {
            cacheData = [XXDiskCache readDataFromDir:directoryPath filename:hash];
            
            if (cacheData) [[XXLRUManager shareManager] refreshIndexOfFileNode:hash];
        }
    }
...
- (void)storeDownloadData:(NSData *)data
    ...
    if (!directoryPath) {
        directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"XXNetworking"] stringByAppendingPathComponent:@"download"];
    ...
    [XXDiskCache writeData:data toDir:directoryPath filename:fileName];
...
- (NSURL *)getDownloadDataFromCacheWithRequestUrl:(NSString *)requestUrl {
    ...
    if (directoryPath) data = [XXDiskCache readDataFromDir:directoryPath filename:fileName];
...
- (NSUInteger)totalCacheSize {
    ...
    return [XXDiskCache dataSizeInDir:diretoryPath];
}
...
- (void)clearDownloadData {
    ...
    [XXDiskCache clearDataIinDir:diretoryPath];
}
...
- (void)clearTotalCache {
    ...
    [XXDiskCache clearDataIinDir:directoryPath];
}

- (void)clearLRUCache {
    if ([self totalCacheSize] > diskCapacity) {
        NSArray *deleteFiles = [[XXLRUManager shareManager] removeLRUFileNodeWithCacheTime:cacheTime];
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        if (directoryPath && deleteFiles.count > 0) {
            [deleteFiles enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [XXDiskCache deleteCache:filePath];
            }];
